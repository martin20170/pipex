...
/*   Updated: 2023/06/10 15:24:55 by mphilip          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	cmd1_process(t_cmds_p_params params, char **path, char **in_n_out, char ***cmds)
{
	...
	if (!cmd1_path)
		free_process(path, in_n_out, cmds, 0);
	close(params.fd[0]);
	params.cmd1_p = fork();
	if (params.cmd1_p == 0)
	{
		cmd1_fd_switch(in_n_out, params.fd);
...
void	cmd2_process(t_cmds_p_params params, char **path, char **in_n_out, char ***cmds)
{
	...
	if (params.cmd2_p == 0)
	{
		cmd2_fd_switch(in_n_out, params.fd);
...
void	cmds_process(char **path, char **in_n_out, char ***cmds)
{
	...
	cmds_p = fork();
	if (cmds_p == 0)
		cmd1_process(cmds_params, path, in_n_out, cmds);
	else
		cmd2_process(cmds_params, path, in_n_out, cmds);
}
